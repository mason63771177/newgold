...
ALTER TABLE `users` 
ADD COLUMN `deposit_address` varchar(50) DEFAULT NULL COMMENT 'TRC20充值地址' AFTER `balance`,
ADD COLUMN `address_derivation_index` int(11) DEFAULT NULL COMMENT '地址派生索引' AFTER `deposit_address`,
ADD COLUMN `tatum_subscription_id` varchar(100) DEFAULT NULL COMMENT 'Tatum监控订阅ID' AFTER `address_derivation_index`,
ADD COLUMN `deposit_address_created_at` timestamp NULL DEFAULT NULL COMMENT '充值地址创建时间' AFTER `tatum_subscription_id`;

-- 为用户表添加索引
ALTER TABLE `users` 
ADD UNIQUE KEY `uk_deposit_address` (`deposit_address`),
ADD UNIQUE KEY `uk_address_derivation_index` (`address_derivation_index`),
ADD KEY `idx_tatum_subscription_id` (`tatum_subscription_id`);
...
ADD COLUMN `deposit_record_id` bigint(20) unsigned DEFAULT NULL COMMENT '关联充值记录ID' AFTER `blockchain_tx_hash`,
