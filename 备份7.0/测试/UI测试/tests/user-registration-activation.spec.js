...
test.describe('用户注册和激活流程', () => {
    ...
    test('用户注册流程', async ({ page }) => {
        ...
        await expect(page).toHaveTitle(/裂金7日/);
    ...
    test('邮箱验证流程', async ({ page, request }) => {
        ...
        const verifyMessage = await page.textContent('.message-success');
    ...
    test('用户登录流程', async ({ page, request }) => {
        ...
        // 等待跳转到钱包页面
         await page.waitForURL('**/wallet.html', { timeout: 10000 });
         
         // 验证登录成功
         expect(page.url()).toContain('wallet.html');
