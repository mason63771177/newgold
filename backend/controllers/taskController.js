      ...
      try {
        ...
        if (user) {
          ...
          try {
            ...
            // 同时记录到tasks表
            await connection.execute(
              'INSERT INTO tasks (user_id, task_type, task_index, status, reward_amount, completed_at, created_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())',
              [userId, 'newbie', taskId, 'completed', rewardAmount]
            );
          } finally {
            connection.release();
          }
      ...
      try {
        ...
        if (user) {
          ...
          try {
            ...
            await connection.execute(
              'INSERT INTO tasks (user_id, task_type, task_index, status, reward_amount, completed_at, created_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())',
              [userId, 'quiz', taskId, 'completed', rewardAmount]
            );
          } finally {
            connection.release();
          }
      ...
      try {
        ...
        if (user) {
          ...
          try {
            ...
            await connection.execute(
              'INSERT INTO tasks (user_id, task_type, task_index, status, reward_amount, completed_at, created_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())',
              [userId, 'master', taskId, 'completed', rewardAmount]
            );
          } finally {
            connection.release();
          }
